<?php
/**
* PageBreakIso Plugin  - Joomla 4.x/5.x plugin
* Version			: 5.0.3
...
class plgcontentphocacountInstallerScript
{
	...
	private $name                    = 'Plugin Content PageBreakIso';
	private $exttype                 = 'plugin';
	private $extname                 = 'pagebreakiso';
	private $previous_version        = '';
	private $dir           = null;
	private $installerName = 'plgcontentpagebreakisoinstaller';
	...
	// disable core pagebreak plugin
		$db = Factory::getDbo();
        $conditions = array(
            $db->qn('type') . ' = ' . $db->q('plugin'),
            $db->qn('element') . ' = ' . $db->quote('pagebreak')
        );
